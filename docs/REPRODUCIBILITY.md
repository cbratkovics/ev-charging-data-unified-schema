...
document, and a second commit adds only those generated files. So an artifact's `code_commit`
...
- **Dedup.** Every survivor choice ends with `_row_hash` as the final tie-break (ADR-0009).
- **Exports.** Rows are written sorted by the grain key; two exports of the same warehouse are
  byte-identical (`tests/test_exports.py`).
...
build, as the idempotency tests and the scheduled workflow do. Compare key-sorted content, never
Parquet bytes, unless the file came from `scripts/export.py`, which sorts.
