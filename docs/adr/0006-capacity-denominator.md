...
- **(f) Robust max.** `ports_inferred = max{k : concurrency k was reached on >= N distinct days}`
  over the station's active life, floored at 1. **N = 5.** Evidence
  (`robust_max_histogram_by_n`, sessions of at least 5 minutes after dedup):
  - Boulder: at N = 1 six station names reach 3 and 32 reach 2; at N = 5 one name reaches 3 and
    35 reach 2; at N = 10 the picture is the same (one at 3, 33 at 2). Level 3 is reached on a
    median of 0 and a 90th-percentile of 1 day (`days_at_level_quantiles`), level 2 on a median
    of 78 days. N = 5 removes the one-day level-3 blips and keeps every dual-port unit.
  - DfT: at N = 1, 466 charge-point ids reach 2 and 30 reach 3 or more; at N = 5, 330 reach 2
    and 5 reach 3; at N = 10, 262 reach 2. Level 2 is reached on a median of only 2 days, so the
    inferred count for DfT is sensitive to N between 2 and 10. That sensitivity is the reason for
    (h), and for DfT the `Connector` id gives a direct port count for the 44% of rows that have
    one (`answers.port.ports_per_station_id`), which is used in preference where present.
  - Stations whose robust max is 0 (fewer than N distinct active days) get 1 port and the flag
...
- **(g) Active window.** A station is available from its first to its last observed session,
  ...
  Boulder 30 days (p99.9 = 32.0 days; 66 gaps over 30 days across 25 station names), DfT 90 days
  (p99.9 = 91.8 days; 634 gaps over 30 days, 237 over 90, across 366 ids). Cary has no end
...
- **(i) Stated limitations.** Model docs, the dbt overview and the README limitations say, in
  ...
  availability is assumed 24 hours a day within the active window; the inference undercounts
...
`ports_inferred * 1440` on active days and 0 on excluded days. Phase 4 builds the sensitivity
