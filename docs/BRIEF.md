...
| 2026-09-20 | post-release | `dim_station` attributes by the majority of sessions with `multi_site_key` / `multi_operator` flags instead of `any_value`; DfT funding-body names trimmed and whitespace-collapsed in silver with the landed spelling kept; the station's operator, not the session's, on station-days and the monthly mart; every ordering closed by a unique key; fixture with an ambiguous station and a byte-identical two-build export test | ADR-0016 |
