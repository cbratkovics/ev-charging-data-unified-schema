...
-- v2: v1 plus two additive interval-outcome columns for recommended entities with an outcome,
-- recommendation_within_band1_rate (reconciled to the artifacts at min_floor = 0, where every
-- entity is recommended) and recommendation_interval_coverage. v1 is what the API serves.
...
metrics as (
    select
        ...
        avg(case when abs(prediction - actual) <= {{ var('within_k')[0] }} then 1.0 else 0.0 end) filter (where policy_action = 'recommend' and actual is not null) as recommendation_within_band1_rate,
        avg(case when prediction_floor <= actual and actual <= prediction_ceiling then 1.0 else 0.0 end) filter (where policy_action = 'recommend' and actual is not null) as recommendation_interval_coverage
    from decisions
...
select
    ...
    cast(recommendation_within_band1_rate as double) as recommendation_within_band1_rate,
