{#- Export every gold model (except meta export=false) to artifacts/marts/<alias>.parquet plus
    _export_manifest.json (model, row count, exported_at_utc, target, invocation id, git commit).
    Files are named by alias so each version of a versioned model gets its own file. Run after
    `dbt build`:  dbt run-operation export_gold --target prod [--args "{out_dir: /tmp/x}"]
    The API reads these files in-process; it never holds a MotherDuck token. -#}
